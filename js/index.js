...
//撕掉标签按钮事件
   ...
   $('.test .btn').click(function(){
    var index=Math.floor(Math.random()*$('.photo-ul li').length);
   $('.photo-ul li').eq(index).show().siblings().hide();
     $('.test').addClass('dn');
     $('.test-result').removeClass('dn');
   })
  function PCDList(){
      var province=new Array();
      for(var i=0;i<provinceList.length;i++){
    
      province.push(`<li>${provinceList[i].name}</li>`);
      }
      $('.sheng').html(province.join(''));
